...
contract ConfigurableRightsPool is PCToken, BalancerOwnable, BalancerReentrancyGuard {
  ...
  function createPool(
    ...
    require(minimumWeightChangeBlockPeriodParam >= addTokenTimeLockInBlocksParam, "ERR_INCONSISTENT_TOKEN_TIME_LOCK");
  ...
  function applyAddToken() external virtual logs lock onlyOwner needsBPool {
    ...
    // Delegate to library to save space
    SmartPoolManager.applyAddToken(IConfigurableRightsPool(address(this)), bPool, addTokenTimeLockInBlocks, newToken);
  }
